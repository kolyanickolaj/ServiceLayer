...
public enum Retry {
    ...
    public enum Action {
        ...
        typealias CompareClosure = (Action, Action) -> Action
    }
    ...
    public struct State {
        ...
        var totalDelay: Retry.Delay

        /// An empty (initial) state.
        static let empty = Retry.State(errors: [], totalDelay: 0)

        /// The total number of attempts made.
        var attemptCount: Int { errors.count + 1 }

        /// The total number of retries made.
        var retryCount: Int { errors.count }
    }
...
extension Retry.Action {
    ...
    static func mostPrioritary(_ lhs: Retry.Action, _ rhs: Retry.Action) -> Retry.Action {
