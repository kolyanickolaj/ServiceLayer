...
protocol URLSessionNetworkStackRepresentable: NetworkStack
where
...
extension Network {
    ...
    public final class URLSessionNetworkStack: NSObject, URLSessionDelegate, URLSessionNetworkStackRepresentable {
        ...
        typealias Resource = URLSessionResource
        typealias Remote = Data
        typealias Response = URLResponse
        typealias FetchError = URLSessionError

        typealias URLSessionDataTaskClosure = (Data?, URLResponse?, Swift.Error?) -> Void

        private let authenticationChallengeHandler: AuthenticationChallengeHandler?
        private let retryQueue: DispatchQueue
        let errorExctracting: ErrorExtracting
        ...
        func finishFetchesAndInvalidateSession() {
        ...
        private func makeTaskCompletionHandler(
            ...
            return { [weak self] data, response, error in
                ...
                let apiCode = JSON(data ?? .init())["code"].intValue
                
                switch (httpStatusCode, apiCode, data) {
                case (.success, 0...210, let remoteData?):
                ...
                case (let statusCode, _, _):
                    self.handleFailedFetch(
                        ...
                        data: data,
                        response: response,
                        error: .server(statusCode, data, urlResponse),
                        cancelableBag: cancelableBag,
                        completion: completion
                    )
                }
